...
class WeightedAStarAgent(Agent):
    ...
    def search(self, env: DragonBallEnv, h_weight) -> Tuple[List[int], float, int]:
        ...
        while open:
            ...
            if curr_node.state not in [item[1].state for item in close.values()]:
                expanded += 1
            
            key = (curr_node.f, curr_node.state[0])
            close[key]=(key, curr_node)
            succ_dict = env.succ(state)
            actions = list(succ_dict.keys())
            for action in actions:
                next_state, cost, terminated = succ_dict[action]
                ...
                if terminated:
                    if env.is_final_state(next_state):
                        child = Node(next_state, curr_node, cost, action)
                        actions, total_cost = self._get_path(child)
                ...
                for key in open.keys():
                    current_state_in_open = open[key][1] # current_state_in_open is a node
                ...
                for key in close.keys():
                    ...
                    if current_state_in_close.state == next_state:
                        existing_state = True
